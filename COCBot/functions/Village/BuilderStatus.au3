...
Func BuilderStatus()
	...
	If ProfileSwitchAccountEnabled() Then
		For $i = 0 To 7
			If $g_aiBuilderTime[$i] = 0 Then $iaLastCheck[$i] = 0
			If $g_aiBuilderTime[$i] > 0 And $iaLastCheck[$i] <> 0 Then
				$g_aiBuilderTime[$i] = Round($g_aiBuilderTime[$i] - TimerDiff($iaLastCheck[$i]) / 60000, 2)
				SetDebugLog("Update Builder Time Acc[" & $i & "]: " & $g_aiBuilderTime[$i] & "m")
				$iaLastCheck[$i] = TimerInit()
			EndIf
		Next
		$g_iBuilderTime = $g_aiBuilderTime[$g_iCurAccount]
		$iLastCheck = $iaLastCheck[$g_iCurAccount]
	EndIf

	Local $iTimeToCheck = _Min($g_iBuilderTime, 6 * 60) - TimerDiff($iLastCheck) / 60000 ; in minutes
	If $g_iBuilderTime = 0 Then $iLastCheck = 0
	If $g_iBuilderTime > 0 And $iLastCheck <> 0 And $iTimeToCheck > 0 Then ;6 hours
		...
		$g_iBuilderTime = Round($g_iBuilderTime - TimerDiff($iLastCheck) / 60000, 2)
